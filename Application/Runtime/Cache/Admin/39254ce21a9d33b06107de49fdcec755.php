...
<div class="layui-fluid">
  <div class="layui-card">
      ...
      <div class="layui-form-item">
        <div class="layui-inline">
          ...
          <div class="layui-input-block">
            <input type="text" name="admin_id" placeholder="请输入" autocomplete="off" class="layui-input">
          </div>
        </div>
        <div class="layui-inline">
          <label class="layui-form-label">管理员账号</label>
          <div class="layui-input-block">
            <input type="text" name="admin_account#like" placeholder="请输入" autocomplete="off" class="layui-input">
          </div>
        </div>
        <div class="layui-inline">
    ...
    <div class="layui-card-body">
      ...
      <table id="LAY-manager-manage" lay-filter="LAY-manager-manage"></table>
...
<script>
    window.Url_Request ={
        url:'<?php echo U("Data/getData");?>&table=admin'
    }
  ...
  layui.config({
  ...
  }).use(['index', 'manager', 'table','admin'], function(){
    var $ = layui.$
            ,form = layui.form
            ,table = layui.table;
    ...
    form.on('submit(LAY-user-front-search)', function(data){
      ...
      //执行重载
      table.reload('LAY-manager-manage', {
        where: field
      });
    });
    ...
    var active = {
      batchdel: function(){
        var checkStatus = table.checkStatus('LAY-manager-manage')
                ,checkData = checkStatus.data; //得到选中的数据
        var idList = "";
        for(var i = 0;i<checkData.length;i++){
          idList += checkData[i].admin_id+",";
        }
        ...
        layer.confirm('确定删除吗？', function(index) {
          //执行 Ajax 后重载
          layui.admin.req({
            url: 'index.php?m=Admin&c=Data&a=deleteData&table=admin&id='+idList,
            method:'get',
      ...
      ,add: function(){
        layer.open({
          type: 2
          ,title: '添加管理员'
          ,content: 'index.php?m=Admin&c=Manager&a=addManager'
          ,maxmin: true
          ,area: ['500px', '450px']
          ,btn: ['确定', '取消']
          ,yes: function(index, layero){
            ...
            iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
              ...
              //执行 Ajax 后重载
              layui.admin.req({
                url: 'index.php?m=Admin&c=Data&a=addData&table=admin',
                method:'post',
                data:field,
                success:function(res){
                  if(res.code==0 && res.success==1){
