    ...
    table.on('tool(LAY-role-manage)', function(obj){
        ...
        if(obj.event === 'del'){
            layer.confirm('确定删除吗？', function(index) {
                //执行 Ajax 后重载
                layui.admin.req({
                    url: 'index.php?m=Admin&c=Data&a=deleteData&table=role&id='+data.role_id,
                    method:'get',
            ...
            layer.open({
                type: 2
                ,title: '编辑角色'
                ,content: 'index.php?m=Admin&c=Role&a=editRole&role_id='+data.role_id
                ,maxmin: true
                ,area: ['500px', '450px']
                ,btn: ['确定', '取消']
                ,yes: function(index, layero){
                    ...
                    iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
                        ...
                        //执行 Ajax 后重载
                        layui.admin.req({
                            url: 'index.php?m=Admin&c=Data&a=editData&table=role',
                            method:'post',
                            data:field,
                            success:function(res){
                                if(res.code==0 && res.success==1){
